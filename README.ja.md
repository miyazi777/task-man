...
- **ファイルプレビュー**: タスク添付ファイル (md / txt) を右ペインに表示
...
| `T` | ゴミ箱ビューから通常リストへ戻る (ゴミ箱ビュー時のみ作用) |
| `p` | カーソル位置のタスクのパスをクリップボードへコピー |
| `R` | ファイル一覧を再読込 (外部での mv / 追加 / 削除を反映) |
| `;` | prefix モード |
| `q` | 終了 |
...
| `p` | パスをコピー (Files 行: ファイルパス / それ以外: タスクディレクトリパス) |
| `R` | ファイル一覧を再読込 |
| `;` | prefix モード |
| `esc` | リストへ戻る |
...
layout:
  main:
    task_list:
      width: 0.6      # 0.1〜0.9 の比率 (画面横幅に対する task list の占有比)
    task_detail:
      height: 0.4    # 0.1〜0.8 の比率 (3 つの height の合計は 1.0 に正規化される)
    file_list:
      height: 0.3    # 0.1〜0.8 の比率
    file_preview:
      height: 0.3    # 0.1〜0.8 の比率
